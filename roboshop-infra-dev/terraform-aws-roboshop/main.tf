resource "aws_alb_target_group" "main" {
  ...
  health_check {
    ...
    interval = 5
    matcher = "200-299"
    path = local.health_check_path # for every backend there is path known as helath whenever we hits on health it will check healthy or not
    port = local.tg_port
    timeout = 2
...
resource "aws_autoscaling_group" "main" {
  ...
  health_check_grace_period = 90  # after 90 sec instance will start 
